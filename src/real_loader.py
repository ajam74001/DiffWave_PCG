'''
This script loads the  and do the segmentions around the S1 and S2 peaks 
...
class Data_loader():
    def __init__(self, path_sig,  resample=True):
...
#         audio , ret['sr'] = librosa.load(f"{path_root}.wav", mono=True, sr=None) # ret['sr'] is the sampling rate 
        try:
            ...
            audio , fs = librosa.load(f"{self.path_sig}/{id}", mono=True, sr=None)
            # print("fs", fs)
        except FileNotFoundError:
            # print(id)
            # print(f"{self.path_sig}/{id}")     
...
wav_file_names = [os.path.basename(file) for file in wav_files]
all_data_seg = []
for index in wav_file_names:
